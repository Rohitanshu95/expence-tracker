  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: '#0f172a', position: 'relative', overflow: 'hidden' }}>
      ...
      <motion.div initial={{ opacity: 0, y: -25 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
        style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '2.5rem 1.5rem 1.5rem', position: 'relative', zIndex: 1, flexShrink: 0 }}>
        <motion.div initial={{ scale: 0, rotate: -180 }} animate={{ scale: 1, rotate: 0 }} transition={{ delay: 0.2, type: 'spring', stiffness: 180, damping: 14 }}
          style={{ width: '64px', height: '64px', borderRadius: '20px', background: 'linear-gradient(135deg, #2563eb, #7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '0.85rem', boxShadow: '0 16px 32px rgba(37,99,235,0.35)', overflow: 'hidden' }}>
          <Logo size={40} />
        </motion.div>
        <motion.h1 initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
          style={{ fontSize: '1.6rem', fontWeight: 900, color: 'white', letterSpacing: '-0.03em', marginBottom: '0.2rem' }}>Welcome Back</motion.h1>
      ...
      <motion.div initial={{ opacity: 0, y: 80 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.12, ease: [0.22, 1, 0.36, 1] }}
        ...
        <motion.div initial={{ width: 0 }} animate={{ width: '36px' }} transition={{ delay: 0.5, duration: 0.3 }}
          style={{ height: '4px', background: '#e2e8f0', borderRadius: '10px', margin: '0 auto 1.25rem', flexShrink: 0 }} />

        <AnimatePresence>
          {error && (
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
              style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.75rem 0.85rem', background: '#fef2f2', color: '#dc2626', borderRadius: '12px', fontSize: '0.8rem', fontWeight: 600, marginBottom: '1rem', border: '1px solid #fecaca', flexShrink: 0 }}>
        ...
        <motion.form onSubmit={submit} variants={stagger} initial="hidden" animate="visible"
          ...
          <motion.div variants={fadeUp} style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
            ...
            <motion.div whileTap={{ scale: 0.995 }} style={{ position: 'relative', borderRadius: '12px', border: focused === 'email' ? '2px solid #2563eb' : '2px solid #f1f5f9', background: '#f8fafc', transition: 'border-color 0.2s' }}>
              ...
              <input type="email" required placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)}
                ...
                style={{ width: '100%', padding: '0.8rem 0.85rem 0.8rem 2.5rem', border: 'none', background: 'transparent', fontSize: '0.9rem', fontWeight: 600, outline: 'none', color: '#1e293b' }} />
            </motion.div>
          </motion.div>
          ...
          <motion.div variants={fadeUp} style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
            ...
            <motion.div whileTap={{ scale: 0.995 }} style={{ position: 'relative', borderRadius: '12px', border: focused === 'pw' ? '2px solid #2563eb' : '2px solid #f1f5f9', background: '#f8fafc', transition: 'border-color 0.2s' }}>
              ...
              <input type={showPw ? 'text' : 'password'} required placeholder="••••••••" value={password}
                ...
                style={{ width: '100%', padding: '0.8rem 2.75rem 0.8rem 2.5rem', border: 'none', background: 'transparent', fontSize: '0.9rem', fontWeight: 600, outline: 'none', color: '#1e293b' }} />
          ...
          <motion.div variants={fadeUp}>
            <motion.button type="submit" disabled={submitting} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
              style={{ width: '100%', background: submitting ? '#94a3b8' : 'linear-gradient(135deg, #2563eb, #1d4ed8)', color: 'white', padding: '0.85rem', borderRadius: '14px', fontWeight: 800, fontSize: '0.95rem', marginTop: '0.25rem', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', border: 'none', cursor: submitting ? 'not-allowed' : 'pointer', boxShadow: '0 6px 20px rgba(37,99,235,0.3)' }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }}
          style={{ paddingTop: '1.25rem', textAlign: 'center', flexShrink: 0 }}>
